...
利用者とGeminiからの入力によって得られたスポットを経由する
...
システムは利用者のカメラ映像から二次元コードを読み取り、
同時にデバイスの位置情報を基に誤差範囲を利用して利用者がスポットの周囲に位置していることを確認する。
...
翌日午前5時に「利用済み」となる。
利用者は画面上でクーポンが「利用可能」なのか、「利用中」なのかを確認できるものとする。
...
本サービスでの利用者の動向を調査し、観光分析に活用できるデータを提供する。
APIへのアクセスにはHTTPリクエストを使用し、レスポンスとしてJSON形式の文字列を返すものとする。

\subsection{APIキーの発行}
APIを利用するには予めサービスに対して事業者登録を行い、
APIキーの発行を行う必要がある。
APIキーの発行はAPIキー管理画面で行うことができる。
システムでは生成したAPIキーをハッシュ化したものを検証用としてデータベースに保存する。

\subsection{アクセスと認証方法}
APIへのアクセスはHTTPを用いて行う。
分析データを取得するためにはリクエストを送信する。
リクエストにはAPIキーと取得したい分析データの期間を指定する必要がある。
リクエストはGETメソッドで\verb|/api/|に
アクセスし、\cref{tab:api-param}に示すパラメータを
渡すことで利用できるものとする。
パラメータの日付はISO 8601の拡張形式を用いる。
また、認証にはBearer認証を用いる。
\begin{table}[H]
	\centering
	\caption{パラメータ}\label{tab:api-param}
	\begin{tabular}{|l|l|l|}
		\hline
		\thead{名前} & \thead{値の例} & \thead{説明} \\ \hline
		\verb|from_date| & 2025-01-01 & 取得したい期間の始点の日付 \\ \hline
		\verb|to_date| & 2025-12-12 & 取得したい期間の終点の日付 \\ \hline
	\end{tabular}
\end{table}
...
分析データは以下の\cref{tab:api-response_root}に示すオブジェクトである。
\begin{table}[H]
	\centering
	\caption{分析データ}\label{tab:api-response_root}
	\begin{tabular}{|l|l|l|}
		\hline
		\thead{キー} & \thead{データ型} & \thead{説明} \\ \hline
		\verb|formatVersion| & number & 分析データのフォーマットバージョン \\ \hline
		\verb|currentUserNum| & number & 現在のサービスの利用者数 \\ \hline
		\verb|spotData| & Array of Object & スポットに関するデータ（参照: \cref{tab:api-response_spots}） \\ \hline
		\verb|couponData| & Array of Object & クーポンに関するデータ（参照: \cref{tab:api-response_coupons}） \\ \hline
	\end{tabular}
\end{table}

\begin{table}[H]
	\centering
	\caption{スポットに関する分析データ}\label{tab:api-response_spots}
	\begin{tabular}{|l|l|l|}
		\hline
		\thead{キー} & \thead{データ型} & \thead{説明} \\ \hline
		\verb|id| & string & スポットのID \\ \hline
		\verb|fromSpotName| & number & 出発地として検索された回数 \\ \hline
		\verb|toSpotName| & number & 到着地として検索された回数 \\ \hline
		\verb|propNum| & number & AIによる提案回数 \\ \hline
	\end{tabular}
\end{table}

\begin{table}[H]
	\centering
	\caption{クーポンに関する分析データ}\label{tab:api-response_coupons}
	\begin{tabular}{|l|l|l|}
		\hline
		\thead{キー} & \thead{データ型} & \thead{説明} \\ \hline
		\verb|id| & string & クーポンのID \\ \hline
		\verb|acquisitionNum| & number & 取得した利用者数 \\ \hline
		\verb|useNum| & number & 使用した利用者数 \\ \hline
	\end{tabular}
\end{table}

\subsection{ログインの失敗}
APIキーが有効でない、またはAPIキーが入力されていない場合はログインに失敗する。
このとき、システムはログインの失敗をクライアントに伝えるために
ステータスコード401 Unauthorizedを返す。

\subsection{アクセス制限}
システムは短時間に複数の無効な資格情報を含むリクエストを検知すると
同一発行元のリクエストに対して10分間、ステータスコード429 Too Many Requestsを返し、
一定時間分析データへのアクセス要求を拒否する。
この機能の実装には、Laravelのレート制限機能を用いる。
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|l|}
		...
		利用者 & MM00 & \verb|User.php| & users \\ \hline
		スポット & MM01 & \verb|Spot.php| & spots \\ \hline
		キーワード & MM02 & \verb|Keyword.php| & keywords \\ \hline
		口コミ & MM03 & \verb|Review.php| & reviews \\ \hline
		クーポン & MM04 & \verb|Coupon.php| & coupons \\ \hline
		利用者クーポン & MM05 & \verb|UserCoupon.php| & user\_coupons \\ \hline
		スタンプ & MM06 & \verb|Stamp.php| & stamps \\ \hline
		検索クエリ & MM07 & \verb|Query.php| & queries \\ \hline
		写真 & MM08 & \verb|Photo.php| & photos \\ \hline
		APIキー & MM09 & \verb|ApiKey.php| & api\_keys \\ \hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	{エラーメッセージ表示画面を構成する}%
	{
		...
		a [label="画面の共通部分出力モジュールを呼び出し"];
		b [label="エラーメッセージ表示部分を構成"];
		c [label="戻るボタンを構成"];
		a -> b -> c;
	}%
	{エラーメッセージ、戻り先URL}%
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|l|}
		...
		画面の共通部分 & MV00 & \verb|layouts/app.blade.php| \\ \hline
		エラーメッセージ表示画面 & MV01 & \verb|error.blade.php| \\ \hline
		ログイン画面 & MV02 & \verb|login.blade.php| \\ \hline
		ホーム画面 & MV03 & \verb|root.blade.php| \\ \hline
		アカウント情報確認画面 & MV04 & \verb|profile.blade.php| \\ \hline
		アカウント情報編集画面 & MV05 & \verb|profile-edit.blade.php| \\ \hline
		事業者申込画面 & MV06 & \verb|subscription-form.blade.php| \\ \hline
		事業者申込完了画面 & MV07 & \verb|subscription-confirm.blade.php| \\ \hline
		アカウント作成画面 & MV08 & \verb|signup.blade.php| \\ \hline
		ログアウト画面 & MV09 & \verb|logout.blade.php| \\ \hline
		スポット詳細画面 & MV10 & \verb|detail.blade.php| \\ \hline
		投稿画面 & MV20 & \verb|photo.blade.php| \\ \hline
		クーポン画面 & MV30 & \verb|coupon.blade.php| \\ \hline
		クーポン使用画面 & MV31 & \verb|coupon-selected.blade.php| \\ \hline
		お楽しみ機能画面 & MV40 & \verb|funpage.blade.php| \\ \hline
		管理者機能ホーム画面 & MV50 & \verb|admin.blade.php| \\ \hline
		利用者一覧画面 & MV51 & \verb|user.blade.php| \\ \hline
		利用者詳細画面 & MV52 & \verb|user-detail.blade.php| \\ \hline
		UGC監視・管理画面 & MV53 & \verb|post.blade.php| \\ \hline
		スポット情報編集画面 & MV54 & \verb|spot.blade.php| \\ \hline
		観光データ確認画面 & MV55 & \verb|data.blade.php| \\ \hline
		事業者機能ホーム画面 & MV56 & \verb|admin.blade.php| \\ \hline
		APIキー管理画面 & MV57 & \verb|api.blade.php| \\ \hline
	\end{tabular}
\end{table}
	...
	{推薦スポット情報、推薦理由メッセージ}%
	{
		\parbox[t]{\dimexpr\textwidth-2\tabcolsep\relax}{
			...
			\begin{itemize}
				...
				\item 検索する範囲は、周辺または出発地・目的地を結ぶ線分を対角線とした矩形の範囲とする。
			\end{itemize}
			\vspace{-.35\zw}
		}
	}
...
\begin{table}[H]
	...
	\modtable{MU14}{app/Http/Controllers/CheckinApiController.php}{送信された二次元コードと位置情報を基にスタンプを付与}
	{
		...
		checkdist [label="距離が誤差範囲内か？", shape=diamond];
...
\begin{table}[H]
	...
	\modtable{MU17}{app/Http/Controllers/CouponApiController.php}
	{クーポンの利用開始時にフロントエンドのプログラムから呼び出される}%
	{
		node [shape=box,style=rounded];
		check_owner [label="利用者が所持しているか確認", shape=diamond];
		check_status [label="利用可能か？", shape=diamond];
		check_expire [label="有効期限内であるか？", shape=diamond];
		gen_key [label="確認用キーの生成とデータベースへの記録"];
		res_ok [label="確認用キーを応答"];
		res_ng [label="エラーメッセージを応答"];
		...
		check_expire -> gen_key [label="Yes"];
		gen_key -> res_ok;
	}
	{クーポンIDを含むHTTPリクエスト}%
	{処理結果、確認用キー}%
	{\parbox[t]{\dimexpr\textwidth-2\tabcolsep\relax}{クーポンを利用する操作をした場合にフロントエンド側からHTTPリクエストを受けて実行される。}}%
\end{table}

\newpage
\subsubsection{投稿マップ構成モジュール}
\begin{table}[H]
	\centering
	\caption{投稿マップ構成モジュール}\label{tab:mod-post-map}
	\modtable{MU19}{app/Http/Controllers/PostMapController.php}%
	{利用者の投稿情報を取得し、地図上に表示するデータを構成する}%
	{
		node [shape=box,style=rounded];
		req [label="リクエスト受信"];
		fetch [label="photosテーブルから位置情報付きの投稿を取得"];
		format [label="データを整形（JSON化）"];
		render [label="投稿マップ画面を構成"];
		req -> fetch;
		fetch -> format;
		format -> render;
	}
...
\newpage
\subsubsection{スポット詳細画面構成モジュール}
\begin{table}[H]
	\centering
	\caption{スポット詳細画面構成モジュール}\label{tab:mod-detail}
	\modtable{MU20}{app/Http/Controllers/detailController.php}%
	{スポット詳細画面の表示に必要なスポット情報および口コミの取得}%
	{
		node [shape=box,style=rounded];
		a [label="スポットの情報を取得"];
		b [label="スポットに対応する口コミを取得"];
		c [label="スポット詳細画面を構成"];
		a->b->c;
	}%
	{利用者から受け取った検索クエリを含むHTTPリクエスト}%
...
\newpage
\subsubsection{APIキー発行処理モジュール}
\begin{table}[H]
	\centering
	\caption{APIキー発行処理モジュール}\label{tab:mod-api-gen}
	\modtable{MA09}{app/Http/Controllers/ApiKeyController.php}%
	{APIキー発行処理を行う}%
	{
		node [shape=box,style=rounded];
		c [label="入力内容が問題あるか？",shape=diamond];
		f [label="エラーメッセージを含むページを応答"];
		g [label="APIキー発行処理を実行"];
		h [label="APIキー管理画面へのリダイレクトを含む応答"];
		c->f [label="Yes"];
		c->g [label="No"];
		g->h;
	}%
	{利用者から受け取ったAPIキー発行に関するHTTPリクエスト}%
	{Webページ}%
	{}
\end{table}
...
\begin{table}[H]
	...
	{チェックインしたスポット}%
	{獲得に成功したかどうか}%
	{\parbox[t]{\dimexpr\textwidth-2\tabcolsep\relax}{スタンプの獲得をトリガとして呼び出される。利用者が同じクーポンを重複して受け取らないようにする。}\vspace{.35\zw}}
\end{table}
