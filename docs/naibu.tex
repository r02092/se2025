...
\subsubsection{経路作成}
ユーザーが入力した出発地・到着地となるスポットから経路を生成する。
また、Gemini APIを利用して、Geminiからスポットの提案を行う。
提案したスポットは出発地から到着地の中継地点として経路に含める。
...
訪れたスポットのスタンプを現地まで赴いた実績として入手できる機能である。
システムはユーザーのカメラ映像から二次元コードを読み取り、
ユーザーの位置情報を基にスポットから半径50\,mの範囲にユーザーが位置しているか検証を行う。
...
システムは逐一、ユーザーのスポット検索の検索内容やユーザーのIDなどを収集し、
...
共通のusersテーブル（\cref{tb:db-users}）で扱う。
...
管理者であるかどうかは、\cref{tb:db-users-permission}に
...
spotsテーブル（\cref{tb:db-spots}）は、
観光地や観光施設、店などのスポットを保存する。
スポットは契約ごとに作成できるため、
どちらのプランに対応するスポットであるかを保存するplanカラムを設けている。
このカラムの値の意味を\cref{tb:db-spots-plan}に示す。
また、\cref{tb:db-spots-type}にカテゴリを表す値の一覧を
示す。keywordsテーブル（\cref{tb:db-keywords}）
スポットにはキーワードを関連付ける。
...
\begin{table}[H]
	\centering
	\caption{spotsテーブルのtypeカラムについて}\label{tb:db-spots-type}
	\begin{tabular}{|r|l|}
		\hline
		\multicolumn{1}{|c|}{\thead{値}} & \thead{意味} \\ \hline
		0 & 飲食 \\ \hline
...
\end{table}
スポットに対する
口コミは、reviewsテーブル（\cref{tb:db-reviews}）に
...
\end{table}
事業者が発行した
クーポンはcouponsテーブル（\cref{tb:db-coupons}）に保存する。
クーポンを手に入れるために行く必要のあるスポットも記録する。
また、利用者が手に入れた
クーポンはuser\_couponsテーブル（\cref{tb:db-user_coupons}）に
...
\end{table}
利用者が手に入れた
スタンプはstampsテーブル（\cref{tb:db-stamps}）に
...
\end{table}
事業者の
発行したAPIキーは、api\_keysテーブル（\cref{tb:db-api_keys}）に
保存する。
...
\newpage
\subsubsection{QRコード読み取りモジュール}
\begin{table}[H]
	\centering
	\caption{QRコード読み取りモジュール}\label{tab:mod-qr}
	\modtable{MC10}{app/Http/Controllers/QrCodeController.php}%
	{カメラ等で読み取った二次元コードを解析し、対応したWebページにリダイレクトする。}%
	{
		node [shape=box,style=rounded];
		c [label="入力内容が問題あるか？",shape=diamond];
		f [label="エラーメッセージを含むページを応答"];
		h [label="QRコード読み取りライブラリでデコード"];
		i [label="Webページを表示"];
		c->f [label="Yes"];
		c->h [label="No"];
		h->i;
	}%
	{利用者から受け取った二次元コードを含むHTTPリクエスト}%
	{Webページ}%
	{スポットのチェックイン用URLや、二要素認証設定用のURIなどを想定する。クライアント側で読み取りを行い、結果をサーバへ送信して処理する方式を想定する。}
\end{table}


\newpage
\subsubsection{二段階認証モジュール}
\begin{table}[H]
	\centering
	\caption{二段階認証（TOTP）モジュール}\label{tab:mod-2fa}
	\modtable{MC12}{app/Http/Controllers/TwoFactorController.php}%
	{利用者がTOTPによる二段階認証を設定・検証・解除するための処理を提供する。}%
	{
		node [shape=box,style=rounded];
    e [shape=diamond];
...
\begin{table}[H]
	...
	{
		node [shape=box,style=rounded];
        input [label="チャット入力と座標情報を受付"];
        calc_bounds [label="検索範囲の計算(現在地周辺 or 出発・目的地の矩形範囲)"];
        search_db [label="座標範囲(緯度・経度)でDBから候補スポットを検索"];
        check_exist [label="候補が存在するか？", shape=diamond];
        prompt [label="候補リストを含めたプロンプト構築"];
        apicall [label="Gemini APIへの送信"];
        check_api [label="APIの応答が成功であるか？", shape=diamond];
	...
	{チャット入力、ユーザー現在地、(任意)目的地座標}%
	{推薦スポット情報（ID含む）、推薦理由メッセージ}%
	{・Geminiへのハルシネーション対策として、DB検索による候補絞り込みを行う。\\
...
\begin{table}[H]
    \centering
    \caption{Google マップ URL生成モジュール}\label{tab:mod-gmap-gen}
    \modtable{MU17}{app/Http/Controllers/GoogleMapUrlController.php}%
    {指定されたスポットを巡るGoogle マップの経路URLを生成する}%
    {
        node [shape=box,style=rounded];
        input [label="出発地・目的地・経由地リストを受付"];
        gemini [label="Gemini提案スポットがあるか？", shape=diamond];
        add_waypoint [label="経由地リストに追加"];
        format_loc [label="各地点を座標(緯度,経度)または名称・住所の形式に整形"];
        ...
        add_waypoint -> format_loc;
        format_loc -> build_query;
        build_query -> build_url;
        build_url -> output;
    }%
    {出発地、目的地、経由地リスト（Gemini提案含む）}%
    {Google マップへの遷移URL}%
    {\parbox[t]{\dimexpr\textwidth-2\tabcolsep\relax}{
        ・経路計算や所要時間の算出は行わず、Google マップへ委譲するURLスキームを生成する。\\
