...
\begin{itemize}
	...
	\begin{itemize}
		\item インデントには、タブを用いる。
		\item セレクタを含む行の末尾の文字は、カンマまたは
		開き波括弧とする。
		\item セレクタに含まれるカンマの後は、改行とする。
		\item セレクタと開き波括弧の間には、
		単一の空白のみを必ず挿入する。
		\item 同じセレクタをネストを用いた記法で
		表現できる場合、ネストを用いた記法を用いる。
		\item ブロックを開始する開き波括弧の後の行は、
		前の行よりインデントを1つ増やす。
		\item 単一の行には、単一の宣言のみを記述する。
		\item プロパティ名と値は、原則として小文字とする。
		固有のフォント名や表示内容に関わるものなどの値は、
		例外的に大文字を使用することを認める。
		\item プロパティ名の直後は、コロンとする。
		\item コロンの後に改行せず値が記述される場合、
		コロンと値の間には単一の空白のみを必ず挿入する。
		\item 全ての宣言の末尾にセミコロンとそれに続く
		改行を挿入する。
		\item 0には単位を付与しないものとする。
		\item ブロックを終了する閉じ波括弧は、
		インデントを除いて他に文字が含まれない行に記述する。
		\item ブロックを終了する閉じ波括弧が含まれる行は、
		前の行よりインデントを1つ減らす。
