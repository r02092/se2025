...
\begin{itemize}
	...
	\begin{itemize}
		...
		そうでない場合は挿入しないものとする。
		\item 可能な場合、Prettierを用いて、コードの
		自動整形をおこなう。
		設定は、ここに記述するルールを可能な限り
		適用するものとする。
		\item 環境変数の命名には、アッパースネークケースを用いる。
	\end{itemize}
	\item \emph{PHPコード（\texttt{*.php}）}
	\begin{itemize}
		...
		\item \verb|App\Http\Controllers\Controller|を継承するクラスの名前の末尾は、\verb|Controller|とする。
		...
		\begin{table}[H]
			...
			\end{tabularx}
		\end{table}
		\item クラスと対応しないファイルの場所は、\cref{sec:dir}で定めた場所とする。
	\end{itemize}
	\item \emph{Bladeテンプレート（\texttt{*.blade.php}）}
	\begin{itemize}
		\item インデントには、タブを用いる。
		...
		\item カスタム属性の名前は、ケバブケースを用いた名前の先頭に“\verb|data-|”を付与したものとする。
		...
		付与したものとする。
	\end{itemize}
	\item \emph{TypeScriptコード（\texttt{*.ts}）}
	\begin{itemize}
		\item インデントには、タブを用いる。
		\item 文は、セミコロンで区切る。
		...
		\item ファイル名は、ローワースネークケースを用いた名前の末尾に“\verb|.ts|”を付与したものとする。
	\end{itemize}
\end{itemize}
...
\subsection{物理ディレクトリ構造}\label{sec:dir}
\begin{verbatim}
