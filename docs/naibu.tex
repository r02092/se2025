...
\begin{table}[H]
    ...
    \modtable{M-USR-03}{QRコードと位置情報によるスタンプ付与}%
    {
        rankdir=TD;
        node [fontname="BIZ UDPGothic", shape=box, style=rounded];
    ...
    {qr\_data (String), user\_lat (Float), user\_lng (Float)}%
    {スタンプ取得結果, クーポン(条件達成時)}%
    {DB座標(整数)を復元してから計算すること。}
\end{table}
