...
さらに、RFC 6238に準拠したTOTPによる二要素認証を
設定できます。
...
利用者は、作成した観光ルートをGoogle Mapsに連携させて表示できます。
...
\begin{figure}[H]
	...
	\caption{パスワード更新}\label{fig:act-edit_account}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{ゲストログイン}\label{fig:act-guest_login}
\end{figure}
...
\begin{figure}[H]
  ...
  \adiagram{利用者,システム,データベース}{1.5}{
	...
	\caption{ユーザ情報追加}\label{fig:act-add_user}
\end{figure}
...
\begin{figure}[H]
  ...
  \adiagram{利用者,システム,データベース}{1.5}{
	...
	\caption{ユーザ情報編集}\label{fig:act-edit_user}
\end{figure}
...
\begin{figure}[H]
  ...
  \adiagram{利用者,システム,データベース}{1.5}{
	...
	\caption{ユーザ情報削除}\label{fig:act-delete_user}
\end{figure}

\subsection{利用者}
\subsubsection{ゲストログイン機能}
...
\begin{figure}[H]
  ...
  \adiagram{利用者,システム,データベース}{1.5}{
    ...
    1/0/最適な経路を計算,
    1/0/計算結果を表示,
    0/0/Google Mapで\\開くボタンをクリック,
    1/0/Google Map連携用\\URLを生成し転送
      }{
    % 不備がある場合の処理
  }
...
\begin{figure}[H]
	\centering
  \adiagram{利用者,システム,AI,データベース}{1.5}{
...
\begin{figure}[H]
  ...
  \adiagram{事業者,システム,データベース}{1.5}{
    ...
    \node[action](fail) at (1, -5.5) {認証失敗の\\エラーを返す};
    \draw[arrow] (action3) -| node[label, pos=0.25] {認証失敗} (fail);
    \draw[arrow] (fail) -- (action7);
  }
...
\begin{figure}[H]
  ...
  \adiagram{事業者,システム,データベース}{1.5}{
    ...
    \node[action](fail) at (1, -7) {不備がある旨を表示};
    \draw[arrow] (action5) -| node[label, pos=0.25] {不備がある} (fail);
  }
