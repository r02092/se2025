...
\begin{figure}[H]
	...
	\adiagram{利用者,システム,データベース}{1.2}{
		...
		% --- 重複エラー時の処理 (分岐2からのNGルート) ---
        \node[action](error2)at(3.5,-11){登録済みである旨\\を表示};
        % 分岐11からエラー表示へ
        \draw[arrow](action11) -| node[label,pos=0.5]{重複あり} (error2);
        % エラー表示から入力(action5)へ戻るループ線 (線が重ならないよう迂回)
        \draw[arrow](error2) |- (action5);
	}
...
\begin{figure}[H]
	...
	\adiagram{利用者,システム,データベース}{1.5}{
		...
		% --- 入力不備時のループ (追加した事前チェックのNGルート) ---
        \node[action](errorInput)at(2.5,-6){不備がある旨\\を表示};
        \draw[arrow](action6) -| node[label,pos=0.5]{不備がある} (errorInput);
        \draw[arrow](errorInput) |- (action3);

        % --- ユーザーが存在しない/パスワード違いのループ ---
        \node[action](errorAuth)at(3.5,-9){認証失敗の旨\\を表示};
        \draw[arrow](action9) -| node[label,pos=0.5]{一致しない} (errorAuth);
        \draw[arrow](errorAuth) |- (action3);
	}
