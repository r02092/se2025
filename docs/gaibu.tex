...
ユーザーがシステムに対して問い合わせを行うための機能です。
問い合わせ内容の入力、送信が可能であり、管理者が対応します。
...
\subsubsection{問い合わせ返信}
\begin{figure}[H]
	\centering
	\adiagram{管理者,システム,データベース}{1.25}{
		0/0/問い合わせ一覧画面を開く,
		1/0/SQLクエリを生成,
		2/0/問い合わせ一覧を検索,
		2/0/結果を返す,
		...
		\node[action](decision1)at(1,-15){返信できない旨を表示};
		\draw[arrow](action10)-|(1.45,-15)node[label,pos=.57]{不備がある}|-(decision1);
		\draw[arrow](decision1)-|(0.45,-8)|-(action8);
	}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node(home)     at(0,  0){ホーム画面};
		\node(acdisplay)at(1,  0){アカウント情報確認};
		\draw[Stealth-Stealth](home)--(acdisplay);
		\node(acup)     at(2,  0){アカウント情報更新};
		\draw[Stealth-Stealth](acdisplay)--(acup);
		\node(acupcomp) at(3,  0){アカウント情報更新完了};
		\draw[       -Stealth](acup)     --(acupcomp);
		\draw[       -Stealth](acupcomp)edge[bend right=45](home);
		\node(businapp) at(2,- 1){事業者申込機能};
		\draw[Stealth-Stealth](acdisplay)|-(businapp);
		\node(login)    at(1,- 2){ログイン画面};
		\draw[Stealth-Stealth](home)     |-(login);
		\node(signup)   at(1,- 3){アカウント作成画面};
		\draw[Stealth-Stealth](home)     |-(signup);
		\node(signupcom)at(2,- 3){アカウント作成完了};
		\draw[       -Stealth](signup)   --(signupcom);
		\draw[       -Stealth](signupcom)edge[bend right=10](home);
		\node(logout)   at(1,- 4){ログアウト画面};
		\draw[Stealth-Stealth](home)     |-(logout);
		\node(posts)    at(1,- 5){投稿};
		\draw[Stealth-Stealth](home)     |-(posts);
		\node(search)   at(1,- 6){観光地・施設検索};
		\draw[Stealth-Stealth](home)     |-(search);
		\node[dashed](g)at(2,- 6){Google マップ経路画面};
		\draw[       -Stealth](search)   --(g);
		\node(coupon)   at(1,- 7){クーポン};
		\draw[Stealth-Stealth](home)     |-(coupon);
		\node(couponuse)at(2,- 7){クーポン使用画面};
		\draw[Stealth-Stealth](coupon)   --(couponuse);
		\node(couponusc)at(3,- 7){クーポン使用完了画面};
		\draw[       -Stealth](couponuse)--(couponusc);
		\draw[       -Stealth](couponusc)edge[bend left=45](coupon);
		\node(funpage)  at(1,- 8){お楽しみ機能};
		\draw[Stealth-Stealth](home)     |-(funpage);
		\node(checkin)  at(2,- 8){チェックイン画面};
		\draw[Stealth-Stealth](funpage)  --(checkin);
		\node(checkinco)at(3,- 8){チェックイン完了画面};
		\draw[       -Stealth](checkin)  --(checkinco);
		\draw[       -Stealth](checkinco)edge[bend left=45](funpage);
		\node(contacts) at(1,- 9){問い合わせ画面};
		\draw[Stealth-Stealth](home)     |-(contacts);
		\node(adminhome)at(1,-10){管理者機能ホーム};
		\draw[Stealth-Stealth](home)     |-(adminhome);
		\node(adminspot)at(2,-10){スポット情報編集画面};
		\draw[Stealth-Stealth](adminhome)--(adminspot);
		\node(adminuser)at(2,-11){ユーザー管理画面};
		\draw[Stealth-Stealth](adminhome)|-(adminuser);
		\node(adminmoni)at(2,-12){UGC監視・管理画面};
		\draw[Stealth-Stealth](adminhome)|-(adminmoni);
		\node(admindata)at(2,-13){観光データ確認画面};
		\draw[Stealth-Stealth](adminhome)|-(admindata);
		\node(admincont)at(2,-14){問い合わせ管理画面};
		\draw[Stealth-Stealth](adminhome)|-(admincont);
		\node(businhome)at(1,-15){事業者機能ホーム};
		\draw[Stealth-Stealth](home)     |-(businhome);
		\node(businspot)at(2,-15){スポット情報編集画面};
		\draw[Stealth-Stealth](businhome)--(businspot);
		\node(businapi) at(2,-16){APIキー発行画面};
		\draw[Stealth-Stealth](businhome)|-(businapi);
		\node(busindata)at(2,-17){観光データ確認画面};
		\draw[Stealth-Stealth](businhome)|-(busindata);
	\end{tikzpicture}
	\caption{画面遷移図}\label{fig:stc}
\end{figure}
本システムのトップページとなる初期画面の
表示例を\cref{fig:ui-home}に示します。
また、ログイン画面およびアカウント作成画面の表示例を
それぞれ\cref{fig:ui-login}および\cref{fig:ui-signup}に
示します。
さらに、主な機能の
表示例を\cref{fig:ui-search}、\cref{fig:ui-post}、
\cref{fig:ui-coupon}、\cref{fig:ui-funpage}に示します。

支払いの必要がある事業者に対し、
システムが出力する請求書の様式を\cref{fig:invoice}に
示します。

\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/home.png}
	\caption{初期画面}\label{fig:ui-home}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/login.png}
	\caption{ログイン画面}\label{fig:ui-login}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/signup.png}
	\caption{アカウント作成画面}\label{fig:ui-signup}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/search.png}
	\caption{検索画面}\label{fig:ui-search}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/post.png}
	\caption{投稿画面}\label{fig:ui-post}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/coupon.png}
	\caption{クーポン使用画面}\label{fig:ui-coupon}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[scale=.23]{ui/funpage.png}
	\caption{お楽しみ機能画面}\label{fig:ui-funpage}
\end{figure}
\newpage
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[bb=16.919437 15.39 689.334588 581.543982,clip,scale=.66]{gaibu-er.pdf}
