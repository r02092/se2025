...
\newcommand\adiagram[4]{
	\begin{tikzpicture}[
		action/.style={draw,thick,rounded corners},
		decision/.style={draw,thick,diamond,inner sep=8pt},
		arrow/.style={-Stealth},
		label/.style={fill=white,inner sep=0pt},
		xscale=5
	]
		...
		\begin{scope}[yscale=#2]
			...
			\foreach\x/\e/\t[count=\i from 1]in{#3}{
				\ifnum\e=0
					\node[action](action\i)at(\x,-\i){\textsf{\t}};
				\else
				...
				\else
					\ifnum\x=\ox--node[label]{\textsf{\ot}}\else-|node{\textsf{\ot}}\fi(action\i);
					\xdef\ot{}
				\fi
...
\begin{figure}[H]
	...
	\adiagram{利用者,システム,データベース}{1.5}{
		...
		\node[action](decision0)at(1,-8){\textsf{存在しない旨を表示}};
		\draw[arrow](action6)-|(1.45,-8)node[label,pos=.57]{\textsf{存在しない}}|-(decision0);
