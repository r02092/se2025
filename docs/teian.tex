...
この課題の解決を目指すものです。
以下に、高知県の公式な課題認識（例：「周遊・滞在促進につながる情報発信・コンテンツが不足」している点など）
\cite{kochi_dx_plan}を踏まえた、
...
\item \emph{観光の一極集中と周遊の不足:}
  ...
  %--周遊不足の裏付け文の追加--%
  このような聖地（点）のみを訪問し、周辺での飲食や物販といった
  ...
  これらの要因により観光が一部地域に集中し、交通渋滞や住民生活への負担など、オーバーツーリズム（観光公害）\cite{kanko_overtourism}の発生が懸念されています。
	...
	「観光客の滞在時間の延長や、地域における消費額の向上を目指して、デジタルデータを活用した観光地域づくりを推進」
	という計画が示されています。
	このことから、聖地巡礼客が「どこから来て」「どこを周り」「どこで消費しているか」
